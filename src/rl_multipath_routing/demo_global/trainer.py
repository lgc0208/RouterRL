...
LastEditTime : 2024-09-23 08:31:07
FilePath     : /root/RouterRL/rl-multipath-routing/src/demo_global/trainer.py
...
sys.path.append(".")
from modules.router_rl.multipath_env import MultipathEnv
    ...
    while step_count < max_ep_steps:
        ...
        if s_or_r == "s":
            ...
            if step < cold_start_steps:
                env.reward_rcvd()
                continue
            delay, loss_rate = map(float, msg.split(","))
