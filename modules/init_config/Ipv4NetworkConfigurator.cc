...
Ipv4NetworkConfigurator::InterfaceInfo::InterfaceInfo(Node *node, LinkInfo *linkInfo,
                                                      NetworkInterface *networkInterface)
    : L3NetworkConfiguratorBase::InterfaceInfo(node, linkInfo, networkInterface), address(0),
      addressSpecifiedBits(0), netmask(0), netmaskSpecifiedBits(0)
{
}
...
Ipv4NetworkConfigurator::RouteInfo *
Ipv4NetworkConfigurator::RoutingTableInfo::findBestMatchingRouteInfo(
    const std::vector<RouteInfo *> &routeInfos, const uint32_t destination, int begin, int end)
{
...
void Ipv4NetworkConfigurator::configureRoutingTable(Node *node)
{
    ...
    for (size_t i = 0; i < node->staticMulticastRoutes.size(); i++) {
        ...
        for (size_t j = 0; j < original->getNumOutInterfaces(); j++)
            clone->addOutInterface(
                new IMulticastRoute::OutInterface(*original->getOutInterface(j)));
...
void Ipv4NetworkConfigurator::configureRoutingTable(Node *node, NetworkInterface *networkInterface)
{
    ...
    for (size_t i = 0; i < node->staticMulticastRoutes.size(); i++) {
        Ipv4MulticastRoute *original = node->staticMulticastRoutes[i];
        bool needed = original->getInInterface()
                      && original->getInInterface()->getInterface() == networkInterface;
        for (size_t j = 0; !needed && j < original->getNumOutInterfaces(); j++)
            if (original->getOutInterface(j)
                && original->getOutInterface(j)->getInterface() == networkInterface)
                needed = true;
 ...
 * | ? | 0 | 1 | ? |
 */
void Ipv4NetworkConfigurator::collectCompatibleInterfaces(
    const std::vector<InterfaceInfo *> &interfaces, /*in*/
    std::vector<Ipv4NetworkConfigurator::InterfaceInfo *>
        &compatibleInterfaces, /*out, and the rest too*/
    uint32_t &mergedAddress, uint32_t &mergedAddressSpecifiedBits,
    uint32_t &mergedAddressIncompatibleBits, uint32_t &mergedNetmask,
    uint32_t &mergedNetmaskSpecifiedBits, uint32_t &mergedNetmaskIncompatibleBits)
{
    EV_TRACE << "Collecting compatible network interfaces from " << interfaces.size()
             << " interfaces: ";
    ...
    for (auto &interface : interfaces) {
        ...
        EV_TRACE << "Trying to merge " << ie->getInterfaceFullPath()
                 << " interface with address specification: " << Ipv4Address(candidateAddress)
                 << " / " << Ipv4Address(candidateAddressSpecifiedBits) << endl;
        EV_TRACE << "Trying to merge " << ie->getInterfaceFullPath()
                 << " interface with netmask specification: " << Ipv4Address(candidateNetmask)
                 << " / " << Ipv4Address(candidateNetmaskSpecifiedBits) << endl;

        // determine merged netmask bits
        uint32_t commonNetmaskSpecifiedBits =
            mergedNetmaskSpecifiedBits & candidateNetmaskSpecifiedBits;
        uint32_t newMergedNetmask =
            mergedNetmask | (candidateNetmask & candidateNetmaskSpecifiedBits);
        uint32_t newMergedNetmaskSpecifiedBits =
            mergedNetmaskSpecifiedBits | candidateNetmaskSpecifiedBits;
        uint32_t newMergedNetmaskIncompatibleBits =
            mergedNetmaskIncompatibleBits
            | ((mergedNetmask & commonNetmaskSpecifiedBits)
               ^ (candidateNetmask & commonNetmaskSpecifiedBits));
        ...
        uint32_t commonAddressSpecifiedBits =
            mergedAddressSpecifiedBits & candidateAddressSpecifiedBits;
        uint32_t newMergedAddress =
            mergedAddress | (candidateAddress & candidateAddressSpecifiedBits);
        uint32_t newMergedAddressSpecifiedBits =
            mergedAddressSpecifiedBits | candidateAddressSpecifiedBits;
        uint32_t newMergedAddressIncompatibleBits =
            mergedAddressIncompatibleBits
            | ((mergedAddress & commonAddressSpecifiedBits)
               ^ (candidateAddress & commonAddressSpecifiedBits));

        // skip interface if there's a bit where the netmask is 1 and the addresses are incompatible
        if ((newMergedNetmask & newMergedNetmaskSpecifiedBits & newMergedAddressIncompatibleBits)
            != 0)
            continue;
        ...
        EV_TRACE << "Merged address specification: " << Ipv4Address(mergedAddress) << " / "
                 << Ipv4Address(mergedAddressSpecifiedBits) << " / "
                 << Ipv4Address(mergedAddressIncompatibleBits) << endl;
        EV_TRACE << "Merged netmask specification: " << Ipv4Address(mergedNetmask) << " / "
                 << Ipv4Address(mergedNetmaskSpecifiedBits) << " / "
                 << Ipv4Address(mergedNetmaskIncompatibleBits) << endl;
    }
    ...
    std::stable_sort(compatibleInterfaces.begin(), compatibleInterfaces.end(),
                     compareInterfaceInfos);
...
void Ipv4NetworkConfigurator::assignAddresses(Topology &topology)
{
    if (configureIsolatedNetworksSeparatly) {
        ...
        EV_INFO << "Assigning network interface address for " << isolatedNetworks.size()
                << " isolated networks separately" << endl;
...
void Ipv4NetworkConfigurator::assignAddresses(std::vector<LinkInfo *> links)
{
    ...
    for (auto &selectedLink : links) {
        EV_INFO << "Assigning network interface addresses on link with "
                << selectedLink->interfaceInfos.size() << " interfaces: ";
        ...
        while (unconfiguredInterfaces.size() != 0) {
            // STEP 1.
            uint32_t
                mergedAddress; // compatible bits of the merged address (both 0 and 1 are address bits)
            uint32_t
                mergedAddressSpecifiedBits; // mask for the valid compatible bits of the merged address (0 means unspecified, 1 means specified)
            uint32_t
                mergedAddressIncompatibleBits; // incompatible bits of the merged address (0 means compatible, 1 means incompatible)
            uint32_t
                mergedNetmask; // compatible bits of the merged netmask (both 0 and 1 are netmask bits)
            uint32_t
                mergedNetmaskSpecifiedBits; // mask for the compatible bits of the merged netmask (0 means unspecified, 1 means specified)
            uint32_t
                mergedNetmaskIncompatibleBits; // incompatible bits of the merged netmask (0 means compatible, 1 means incompatible)
            std::vector<InterfaceInfo *> compatibleInterfaces; // the list of compatible interfaces
            collectCompatibleInterfaces(unconfiguredInterfaces, compatibleInterfaces, mergedAddress,
                                        mergedAddressSpecifiedBits, mergedAddressIncompatibleBits,
                                        mergedNetmask, mergedNetmaskSpecifiedBits,
                                        mergedNetmaskIncompatibleBits);
            ...
            int minimumNetmaskLength =
                bitSize
                - getLeastSignificantBitIndex(
                    mergedNetmask & mergedNetmaskSpecifiedBits, 1,
                    bitSize); // 0 means 0.0.0.0, bitSize means 255.255.255.255
            int maximumNetmaskLength =
                bitSize - 1
                - getMostSignificantBitIndex(~mergedNetmask & mergedNetmaskSpecifiedBits, 1,
                                             -1); // 0 means 0.0.0.0, bitSize means 255.255.255.255
            maximumNetmaskLength = std::min(
                maximumNetmaskLength,
                bitSize - 1 - getMostSignificantBitIndex(mergedAddressIncompatibleBits, 1, -1));
            ...
            maximumNetmaskLength =
                std::min(maximumNetmaskLength, bitSize - interfaceAddressBitCount);
            EV_TRACE << "Netmask valid length range: " << minimumNetmaskLength << " - "
                     << maximumNetmaskLength << " bits" << endl;
            ...
            for (netmaskLength = maximumNetmaskLength; netmaskLength >= minimumNetmaskLength;
                 netmaskLength--) {
                ASSERT(netmaskLength < bitSize);
                networkNetmask = ~(~((uint32_t)0) >> netmaskLength);
                EV_TRACE << "Trying network netmask: " << Ipv4Address(networkNetmask) << " : "
                         << netmaskLength << " bits" << endl;
                networkAddress = mergedAddress & mergedAddressSpecifiedBits & networkNetmask;
                uint32_t networkAddressUnspecifiedBits =
                    ~mergedAddressSpecifiedBits
                    & networkNetmask; // 1 means the network address unspecified
                uint32_t networkAddressUnspecifiedPartLimit =
                    getPackedBits(~(uint32_t)0, networkAddressUnspecifiedBits) + (uint32_t)1;
                EV_TRACE << "Counting from: " << 0 << " to: " << networkAddressUnspecifiedPartLimit
                         << endl;

                // we start with +1 so that the network address will be more likely different
                for (uint32_t networkAddressUnspecifiedPart = 0;
                     networkAddressUnspecifiedPart <= networkAddressUnspecifiedPartLimit;
                     networkAddressUnspecifiedPart++) {
                    networkAddress = setPackedBits(networkAddress, networkAddressUnspecifiedBits,
                                                   networkAddressUnspecifiedPart);
                    ...
                    if (assignDisjunctSubnetAddressesParameter) {
                        ...
                        for (int i = 0; i < (int)assignedNetworkAddresses.size(); i++) {
                            ...
                            uint32_t assignedNetworkAddressMaximum =
                                assignedNetworkAddress | ~assignedNetworkNetmask;
                            if (networkAddress <= assignedNetworkAddressMaximum
                                && assignedNetworkAddress <= networkAddressMaximum)
                                overlaps = true;
                        }
                        if (overlaps)
                            continue;
                    ...
                    if ((1 << (bitSize - netmaskLength))
                        >= interfaceCount + compatibleInterfaceCount)
                        goto found;
                }
            }
        found:
            if (netmaskLength < minimumNetmaskLength || netmaskLength > maximumNetmaskLength)
                throw cRuntimeError(
                    "Failed to find address prefix (using %s with specified bits %s) and netmask "
                    "(length from %d bits to %d bits) for interface %s and %zu other interface(s). "
                    "Please refine your parameters and try again!",
                    Ipv4Address(mergedAddress).str().c_str(),
                    Ipv4Address(mergedAddressSpecifiedBits).str().c_str(), minimumNetmaskLength,
                    maximumNetmaskLength,
                    compatibleInterfaces[0]->networkInterface->getInterfaceFullPath().c_str(),
            ...
            for (auto &compatibleInterface : compatibleInterfaces) {
                ...
                uint32_t interfaceAddressUnspecifiedBits =
                    ~interfaceAddressSpecifiedBits
                    & ~networkNetmask; // 1 means the interface address is unspecified
                uint32_t interfaceAddressUnspecifiedPartMaximum = 0;
                for (auto &assignedInterfaceAddress : assignedInterfaceAddresses) {
                    uint32_t otherInterfaceAddress = assignedInterfaceAddress;
                    if ((otherInterfaceAddress & ~interfaceAddressUnspecifiedBits)
                        == ((networkAddress | interfaceAddress)
                            & ~interfaceAddressUnspecifiedBits)) {
                        uint32_t otherInterfaceAddressUnspecifiedPart =
                            getPackedBits(otherInterfaceAddress, interfaceAddressUnspecifiedBits);
                        if (otherInterfaceAddressUnspecifiedPart
                            > interfaceAddressUnspecifiedPartMaximum)
                            interfaceAddressUnspecifiedPartMaximum =
                                otherInterfaceAddressUnspecifiedPart;
                    }
                }
                interfaceAddressUnspecifiedPartMaximum++;
                interfaceAddress = setPackedBits(interfaceAddress, interfaceAddressUnspecifiedBits,
                                                 interfaceAddressUnspecifiedPartMaximum);
                ...
                if (assignUniqueAddresses
                    && containsKey(assignedAddressToNetworkInterfaceMap, completeAddress))
                    throw cRuntimeError("Failed to configure unique address for %s. Please refine "
                                        "your parameters and try again!",
                                        networkInterface->getInterfaceFullPath().c_str());
                assignedAddressToNetworkInterfaceMap[completeAddress] =
                    compatibleInterface->networkInterface;
                assignedInterfaceAddresses.push_back(completeAddress);

                // configure interface with the selected address and netmask
                EV_TRACE << "Setting interface address, interface = "
                         << compatibleInterface->getFullPath()
                         << ", address = " << Ipv4Address(completeAddress)
                         << ", netmask = " << Ipv4Address(completeNetmask) << endl;
                if (routingMode == "convention") {
                    std::string fullPath = compatibleInterface->getFullPath();
                    size_t firstDot = fullPath.find('.');
                    size_t secondDot = fullPath.find('.', firstDot + 1);
                    std::string deviceName =
                        fullPath.substr(firstDot + 1, secondDot - firstDot - 1);
                    std::string address = Ipv4Address(completeAddress).str();
                    RlConventionalRoutingTable::getInstance()->setDeviceAddress(deviceName,
                                                                                address);
                }
...
Ipv4NetworkConfigurator::InterfaceInfo *
Ipv4NetworkConfigurator::createInterfaceInfo(L3NetworkConfiguratorBase::Topology &topology,
                                             L3NetworkConfiguratorBase::Node *node,
                                             LinkInfo *linkInfo, NetworkInterface *ie)
{
    InterfaceInfo *interfaceInfo =
        new InterfaceInfo(static_cast<Ipv4NetworkConfigurator::Node *>(node), linkInfo, ie);
...
void Ipv4NetworkConfigurator::readInterfaceConfiguration(Topology &topology)
{
    ...
    EV_INFO << "Reading network interface configuration of " << interfaceElements.size()
            << " XML elements" << endl;
    ...
    for (auto &interfaceElement : interfaceElements) {
        ...
        const char *interfaceAttr =
            interfaceElement->getAttribute("names"); // i.e. interface names, like "eth* ppp0"

        // TODO "switch" egyebkent sztem nem muxik most, de kellene!
        const char *towardsAttr =
            interfaceElement->getAttribute("towards"); // neighbor host names, like "ap switch"
        const char *amongAttr =
            interfaceElement->getAttribute("among"); // neighbor host names, like "host[*] router1"
        ...
        const char *groupsAttr =
            interfaceElement->getAttribute("groups"); // list of multicast addresses
        bool addStaticRouteAttr = getAttributeBoolValue(interfaceElement, "add-static-route", true);
        bool addDefaultRouteAttr =
            getAttributeBoolValue(interfaceElement, "add-default-route", true);
        ...
        try {
            ...
            for (auto &linkInfo : topology.linkInfos) {
                for (int j = 0; j < (int)linkInfo->interfaceInfos.size(); j++) {
                    InterfaceInfo *interfaceInfo =
                        static_cast<InterfaceInfo *>(linkInfo->interfaceInfos[j]);
                    if (interfacesSeen.count(interfaceInfo) == 0) {
                        cModule *hostModule =
                            interfaceInfo->networkInterface->getInterfaceTable()->getHostModule();
                        std::string hostFullPath = hostModule->getFullPath();
                        std::string hostShortenedFullPath =
                            hostFullPath.substr(hostFullPath.find('.') + 1);

                        // Note: "hosts", "interfaces" and "towards" must ALL match on the interface for the rule to apply
                        if ((hostMatcher.matchesAny()
                             || hostMatcher.matches(hostShortenedFullPath.c_str())
                             || hostMatcher.matches(hostFullPath.c_str()))
                            && (interfaceMatcher.matchesAny()
                                || interfaceMatcher.matches(
                                    interfaceInfo->networkInterface->getInterfaceName()))
                            && (towardsMatcher.matchesAny()
                                || linkContainsMatchingHostExcept(linkInfo, &towardsMatcher,
                                                                  hostModule))) {
                            EV_DEBUG << "Processing interface configuration for "
                                     << interfaceInfo->getFullPath() << " using "
                                     << interfaceElement->str() << endl;
                            ...
                            if (!opp_isempty(groupsAttr)) {
                                ...
                                while (tokenizer.hasMoreTokens())
                                    interfaceInfo->multicastGroups.push_back(
                                        Ipv4Address(tokenizer.nextToken()));
                            }
            ...
            throw cRuntimeError("Error in XML <interface> element at %s: %s",
                                interfaceElement->getSourceLocation(), e.what());
        }
    }
}

void Ipv4NetworkConfigurator::parseAddressAndSpecifiedBits(const char *addressAttr,
                                                           uint32_t &outAddress,
                                                           uint32_t &outAddressSpecifiedBits)
{
    ...
    if (!Ipv4Address::isWellFormed(address.c_str())
        || !Ipv4Address::isWellFormed(specifiedBits.c_str()))
...
bool Ipv4NetworkConfigurator::linkContainsMatchingHostExcept(LinkInfo *linkInfo,
                                                             Matcher *hostMatcher,
                                                             cModule *exceptModule)
{
    for (auto &element : linkInfo->interfaceInfos) {
        ...
        if (hostMatcher->matches(hostShortenedFullPath.c_str())
            || hostMatcher->matches(hostFullPath.c_str()))
            return true;
    }
    return false;
...
void Ipv4NetworkConfigurator::dumpLinks(Topology &topology)
{
    ...
    for (size_t i = 0; i < topology.linkInfos.size(); i++) {
        ...
        if (linkInfo->gatewayInterfaceInfo)
            EV_INFO << " uses gateway "
                    << linkInfo->gatewayInterfaceInfo->networkInterface->getInterfaceFullPath();
        EV_INFO << endl;
        for (auto &element : linkInfo->interfaceInfos) {
            InterfaceInfo *interfaceInfo = static_cast<InterfaceInfo *>(element);
    ...
    for (int i = 0; i < topology.getNumNodes(); i++) {
        ...
        if (node->routingTable) {
            ...
            if (node->routingTable->getNumMulticastRoutes() > 0)
                check_and_cast<IIpv4RoutingTable *>(node->routingTable)
...
void Ipv4NetworkConfigurator::dumpConfig(Topology &topology)
{
    ...
    for (auto &linkInfo : topology.linkInfos) {
        for (auto &element : linkInfo->interfaceInfos) {
            ...
            stream << "   <interface hosts=\"" << interfaceInfo->node->module->getFullPath()
                   << "\" names=\"" << networkInterface->getInterfaceName() << "\" address=\""
                   << interfaceData->getIPAddress() << "\" netmask=\""
                   << interfaceData->getNetmask() << "\" metric=\"" << interfaceData->getMetric()
                   << "\"/>" << endl;
    ...
    for (auto &linkInfo : topology.linkInfos) {
        for (auto &element : linkInfo->interfaceInfos) {
            ...
            if (numOfMulticastGroups > 0) {
                std::stringstream stream;
                stream << "   <multicast-group hosts=\""
                       << interfaceInfo->node->module->getFullPath() << "\" interfaces=\""
                       << networkInterface->getInterfaceName() << "\" address=\"";
    ...
    for (auto &linkInfo : topology.linkInfos) {
        ...
        if (hasWireless) {
            ...
            for (auto &element : linkInfo->interfaceInfos) {
                ...
                InterfaceInfo *interfaceInfo = static_cast<InterfaceInfo *>(element);
                if (interfaceInfo->networkInterface->isWireless()) {
                    stream << interfaceInfo->node->module->getFullPath() << "%"
                           << interfaceInfo->networkInterface->getInterfaceName();
                    first = false;
                }
            }
    ...
    for (int i = 0; i < topology.getNumNodes(); i++) {
        ...
        if (routingTable) {
            for (int j = 0; j < routingTable->getNumRoutes(); j++) {
                ...
                stream << "\" interface=\"" << route->getInterfaceName() << "\" metric=\""
...
void Ipv4NetworkConfigurator::readMulticastGroupConfiguration(Topology &topology)
{
    ...
    EV_INFO << "Reading multicast group configuration of " << multicastGroupElements.size()
            << " XML elements" << endl;
    for (auto &multicastGroupElement : multicastGroupElements) {
        ...
        const char *towardsAttr =
            multicastGroupElement->getAttribute("towards"); // neighbor host names, like "ap switch"
        ...
        if (amongAttr && *amongAttr) { // among="X Y Z" means hosts = "X Y Z" towards = "X Y Z"
            if ((hostAttr && *hostAttr) || (towardsAttr && *towardsAttr))
                throw cRuntimeError(
                    "The 'hosts'/'towards' and 'among' attributes are mutually exclusive, at %s",
                    multicastGroupElement->getSourceLocation());
            towardsAttr = hostAttr = amongAttr;
        }
        ...
        try {
            ...
            while (tokenizer.hasMoreTokens()) {
                ...
                if (!addr.isMulticast())
                    throw cRuntimeError(
                        "Non-multicast address %s found in the multicast-group element",
                        addr.str().c_str());
            ...
            for (auto &linkInfo : topology.linkInfos) {
                for (size_t k = 0; k < linkInfo->interfaceInfos.size(); k++) {
                    InterfaceInfo *interfaceInfo =
                        static_cast<InterfaceInfo *>(linkInfo->interfaceInfos[k]);
                    cModule *hostModule =
                        interfaceInfo->networkInterface->getInterfaceTable()->getHostModule();
                    std::string hostFullPath = hostModule->getFullPath();
                    std::string hostShortenedFullPath =
                    ...
                    if ((hostMatcher.matchesAny()
                        ...
                        for (auto &multicastGroup : multicastGroups) {
                            EV_INFO << "Adding multicast group " << multicastGroup << " to "
                                    << interfaceInfo->networkInterface->getInterfaceFullPath()
                                    << endl;
            ...
            throw cRuntimeError("Error in XML <multicast-group> element at %s: %s",
                                multicastGroupElement->getSourceLocation(), e.what());
        }
    }
}
...
void Ipv4NetworkConfigurator::readManualRouteConfiguration(Topology &topology)
{
    ...
    EV_INFO << "Reading manual route configuration of " << routeElements.size() << " XML elements"
            << endl;
    for (auto &routeElement : routeElements) {
        const char *hostAttr = xmlutils::getMandatoryFilledAttribute(*routeElement, "hosts");
        const char *destinationAttr = xmlutils::getMandatoryAttribute(
            *routeElement, "destination"); // destination address  (L3AddressResolver syntax)
        const char *netmaskAttr = routeElement->getAttribute(
            "netmask"); // default: 255.255.255.255; alternative notation: "/23"
        const char *gatewayAttr =
            routeElement->getAttribute("gateway"); // next hop address (L3AddressResolver syntax)
        const char *interfaceAttr =
            routeElement->getAttribute("interface"); // output interface name
        ...
        try {
            ...
            if (opp_isempty(interfaceAttr) && opp_isempty(gatewayAttr))
                throw cRuntimeError(
                    "Incomplete route: either gateway or interface (or both) must be specified");
            ...
            for (int i = 0; i < topology.getNumNodes(); i++) {
                ...
                if (node->routingTable) {
                    ...
                    std::string hostShortenedFullPath =
                        hostFullPath.substr(hostFullPath.find('.') + 1);
                    if (atMatcher.matches(hostShortenedFullPath.c_str())
                        || atMatcher.matches(hostFullPath.c_str())) {
                        // determine the gateway (its address towards this node!) and the output interface for the route (must be done per node)
                        NetworkInterface *ie;
                        Ipv4Address gateway;
                        resolveInterfaceAndGateway(node, interfaceAttr, gatewayAttr, ie, gateway,
                                                   topology);
                        ...
                        EV_INFO << "Adding manual route " << *route << " to "
                                << node->module->getFullPath() << endl;
                        node->staticRoutes.push_back(route);
                    }
                }
            }
        } catch (std::exception &e) {
            throw cRuntimeError("Error in XML <route> element at %s: %s",
                                routeElement->getSourceLocation(), e.what());
        }
    }
}
...
void Ipv4NetworkConfigurator::readManualMulticastRouteConfiguration(Topology &topology)
{
    ...
    EV_INFO << "Reading multicast route configuration of " << routeElements.size()
            << " XML elements" << endl;
    for (auto &routeElement : routeElements) {
        const char *hostAttr = routeElement->getAttribute("hosts");
        const char *sourceAttr =
            routeElement->getAttribute("source"); // source address  (L3AddressResolver syntax)
        const char *netmaskAttr = routeElement->getAttribute(
            "netmask"); // default: 255.255.255.255; alternative notation: "/23"
        const char *groupsAttr = routeElement->getAttribute(
            "groups"); // addresses of the multicast groups, default: 0.0.0.0, matching all groups
        const char *parentAttr =
            routeElement->getAttribute("parent"); // name of expected input interface
        const char *childrenAttr =
            routeElement->getAttribute("children"); // names of output interfaces
        const char *metricAttr = routeElement->getAttribute("metric");
        ...
        try {
            ...
            if (opp_isempty(groupsAttr))
                ...
                while (tokenizer.hasMoreTokens()) {
                    ...
                    if (!group.isMulticast())
                        throw cRuntimeError("Address '%s' in groups attribute is not multicast.",
                                            group.str().c_str());
                    ...
                    if (atMatcher.matches(hostShortenedFullPath.c_str())
                        ...
                        if (!opp_isempty(parentAttr)) {
                            ...
                            if (!parent->isMulticast())
                                throw cRuntimeError("Parent interface '%s' is not multicast.",
                                                    parentAttr);
                        }
                        ...
                        for (auto &element : node->interfaceInfos) {
                            ...
                            if (ie != parent && ie->isMulticast()
                                && childrenMatcher.matches(interfaceInfo))
                        ...
                        for (auto &group : groups) {
                            ...
                            route->setInInterface(
                                parent ? new Ipv4MulticastRoute::InInterface(parent) : nullptr);
                            if (!opp_isempty(metricAttr))
                                route->setMetric(atoi(metricAttr));
                            for (auto &child : children)
                                route->addOutInterface(new Ipv4MulticastRoute::OutInterface(
                                    child, false /*TODOisLeaf*/));
                            EV_INFO << "Adding manual multicast route " << *route << " to "
                                    << node->module->getFullPath() << endl;
            ...
            throw cRuntimeError("Error in XML <multicast-route> element at %s: %s",
                                routeElement->getSourceLocation(), e.what());
        }
    }
}

void Ipv4NetworkConfigurator::resolveInterfaceAndGateway(Node *node, const char *interfaceAttr,
                                                         const char *gatewayAttr,
                                                         NetworkInterface *&outIE,
                                                         ...
                                                         Topology &topology)
{
    ...
    if (!outIE) {
        ...
        for (auto &linkInfo : topology.linkInfos) {
            InterfaceInfo *gatewayInterfaceOnLink =
                findInterfaceOnLinkByNodeAddress(linkInfo, outGateway);
            if (gatewayInterfaceOnLink) {
                InterfaceInfo *nodeInterfaceOnLink =
                    findInterfaceOnLinkByNode(linkInfo, node->module);
...
Ipv4NetworkConfigurator::InterfaceInfo *
Ipv4NetworkConfigurator::findInterfaceOnLinkByNode(LinkInfo *linkInfo, cModule *node)
{
    for (auto &element : linkInfo->interfaceInfos) {
        InterfaceInfo *interfaceInfo = static_cast<InterfaceInfo *>(element);
...
Ipv4NetworkConfigurator::InterfaceInfo *
Ipv4NetworkConfigurator::findInterfaceOnLinkByNodeAddress(LinkInfo *linkInfo, Ipv4Address address)
{
    for (auto &element : linkInfo->interfaceInfos) {
...
Ipv4NetworkConfigurator::LinkInfo *
Ipv4NetworkConfigurator::findLinkOfInterface(Topology &topology, NetworkInterface *ie)
{
    for (auto &linkInfo : topology.linkInfos) {
...
bool Ipv4NetworkConfigurator::containsRoute(const std::vector<Ipv4Route *> &routes,
                                            Ipv4Route *route)
{
    for (auto &rt : routes)
        if (*rt == *route)
...
void Ipv4NetworkConfigurator::addStaticRoutes(Topology &topology, cXMLElement *autorouteElement)
{
    ...
    for (int i = 0; i < topology.getNumNodes(); i++) {
        ...
        for (auto &nodeElement : nodeElements) {
            ...
            std::string hostFullPath = node->module->getFullPath();
            std::string hostShortenedFullPath = hostFullPath.substr(hostFullPath.find('.') + 1);
            if (nodeHostsMatcher.matchesAny()
                || nodeHostsMatcher.matches(hostShortenedFullPath.c_str())
                || nodeHostsMatcher.matches(hostFullPath.c_str())) {
                selectedNodeElement = nodeElement;
                break;
            }
        }
        double weight = computeNodeWeight(node, metric, selectedNodeElement);
        EV_DEBUG << "Setting node weight, node = " << node->module->getFullPath()
                 << ", weight = " << weight << endl;
    ...
    for (int i = 0; i < topology.getNumNodes(); i++) {
        ...
        for (int j = 0; j < node->getNumInLinks(); j++) {
            ...
            for (auto &linkElement : linkElements) {
                ...
                std::string sourceShortenedFullPath =
                    sourceFullPath.substr(sourceFullPath.find('.') + 1);
                std::string destinationFullPath = link->destinationInterfaceInfo->getFullPath();
                std::string destinationShortenedFullPath =
                    destinationFullPath.substr(destinationFullPath.find('.') + 1);
                if (linkInterfaceMatcher.matchesAny()
                    || linkInterfaceMatcher.matches(sourceFullPath.c_str())
                    || linkInterfaceMatcher.matches(sourceShortenedFullPath.c_str())
    ...
    for (int i = 0; i < topology.getNumNodes(); i++) {
        ...
        std::string hostShortenedFullPath = hostFullPath.substr(hostFullPath.find('.') + 1);
        if (!sourceHostsMatcher.matchesAny()
            && !sourceHostsMatcher.matches(hostShortenedFullPath.c_str())
            && !sourceHostsMatcher.matches(hostFullPath.c_str()))
            continue;
        ...
        if (addDefaultRoutesParameter && sourceNode->interfaceInfos.size() == 1
            && sourceNode->interfaceInfos[0]->linkInfo->gatewayInterfaceInfo
            && sourceNode->interfaceInfos[0]->addDefaultRoute) {
            InterfaceInfo *sourceInterfaceInfo =
                static_cast<InterfaceInfo *>(sourceNode->interfaceInfos[0]);
            NetworkInterface *sourceNetworkInterface = sourceInterfaceInfo->networkInterface;
            InterfaceInfo *gatewayInterfaceInfo =
                static_cast<InterfaceInfo *>(sourceInterfaceInfo->linkInfo->gatewayInterfaceInfo);
            ...
            if (addDirectRoutesParameter) {
                ...
                route->setDestination(
                    sourceInterfaceInfo->getAddress().doAnd(sourceInterfaceInfo->getNetmask()));
                ...
                EV_DEBUG << "Adding direct route " << *route << " to "
                         << sourceNode->module->getFullPath() << endl;
            ...
            route->setInterface(sourceNetworkInterface);
            route->setSourceType(Ipv4Route::MANUAL);
            EV_DEBUG << "Adding default route " << *route << " to "
                     << sourceNode->module->getFullPath() << endl;
            sourceNode->staticRoutes.push_back(route);

            // skip building and optimizing the whole routing table
            EV_DEBUG << "Adding default routes to " << sourceNode->getModule()->getFullPath()
                     << ", node has only one (non-loopback) interface" << endl;
        } else {
            ...
            for (int j = 0; j < topology.getNumNodes(); j++) {
                ...
                while (node != sourceNode) {
                    ...
                    if (node != sourceNode && !isBridgeNode(node) && link->sourceInterfaceInfo)
                        nextHopInterfaceInfo =
                            static_cast<InterfaceInfo *>(link->sourceInterfaceInfo);
                    node = (Node *)node->getPath(0)->getLinkOutRemoteNode();
                }

                // determine source interface
                if (nextHopInterfaceInfo && link->destinationInterfaceInfo
                    && link->destinationInterfaceInfo->addStaticRoute) {
                    NetworkInterface *sourceNetworkInterface =
                        link->destinationInterfaceInfo->networkInterface;
                    ...
                    for (size_t j = 0; j < destinationNode->interfaceInfos.size(); j++) {
                        InterfaceInfo *destinationInterfaceInfo =
                            static_cast<InterfaceInfo *>(destinationNode->interfaceInfos[j]);
                        std::string destinationFullPath =
                            destinationInterfaceInfo->networkInterface->getInterfaceFullPath();
                        std::string destinationShortenedFullPath =
                            destinationFullPath.substr(destinationFullPath.find('.') + 1);
                        if (!destinationInterfacesMatcher.matchesAny()
                            && !destinationInterfacesMatcher.matches(destinationFullPath.c_str())
                            && !destinationInterfacesMatcher.matches(
                                destinationShortenedFullPath.c_str()))
                            continue;
                        NetworkInterface *destinationNetworkInterface =
                            destinationInterfaceInfo->networkInterface;
                        ...
                        if (!destinationNetworkInterface->isLoopback()
                            && !destinationAddress.isUnspecified()) {
                            Ipv4Route *route = new Ipv4Route();
                            Ipv4Address gatewayAddress = nextHopInterfaceInfo->getAddress();
                            if (addSubnetRoutesParameter
                                && destinationNode->interfaceInfos.size() == 1
                                && destinationNode->interfaceInfos[0]
                                       ->linkInfo->gatewayInterfaceInfo
                                && destinationNode->interfaceInfos[0]->addSubnetRoute) {
                                ASSERT(
                                    !destinationAddress.doAnd(destinationNetmask).isUnspecified());
                            ...
                            if (containsRoute(sourceNode->staticRoutes, route))
                                delete route;
                            else if (!addDirectRoutesParameter
                                     && route->getGateway().isUnspecified())
                                delete route;
                            else {
                                sourceNode->staticRoutes.push_back(route);
                                EV_DEBUG << "Adding route "
                                         << sourceNetworkInterface->getInterfaceFullPath() << " -> "
                                         << destinationNetworkInterface->getInterfaceFullPath()
                                         << " as " << route->str() << endl;
                            }
                        }
                    }
            ...
            if (optimizeRoutesParameter) {
                EV_INFO << "Optimizing " << sourceNode->staticRoutes.size() << " routes for "
                        << sourceNode->module->getFullName();
...
{
    return route1->getSourceType() == route2->getSourceType()
           && route1->getMetric() == route2->getMetric()
           && route1->getGateway() == route2->getGateway()
           && route1->getInterface() == route2->getInterface();
}

/**
 * Returns the index of the first route that has the same color.
 */
int Ipv4NetworkConfigurator::findRouteIndexWithSameColor(const std::vector<Ipv4Route *> &routes,
                                                         Ipv4Route *route)
{
 ...
 */
bool Ipv4NetworkConfigurator::routesCanBeNeighbors(const std::vector<RouteInfo *> &routeInfos,
                                                   int i, int j)
{
 ...
 */
bool Ipv4NetworkConfigurator::interruptsOriginalRoute(const RoutingTableInfo &routingTableInfo,
                                                      int begin, int end,
                                                      RouteInfo *originalRouteInfo)
{
    Ipv4NetworkConfigurator::RouteInfo *matchingRouteInfo =
        routingTableInfo.findBestMatchingRouteInfo(originalRouteInfo->destination, begin, end);
 ...
 */
bool Ipv4NetworkConfigurator::interruptsAnyOriginalRoute(
    const RoutingTableInfo &routingTableInfo, int begin, int end,
 ...
 */
bool Ipv4NetworkConfigurator::interruptsSubsequentOriginalRoutes(
    const RoutingTableInfo &routingTableInfo, int index)
{
    for (size_t i = index + 1; i < routingTableInfo.routeInfos.size(); i++) {
        ...
        if (interruptsAnyOriginalRoute(routingTableInfo, index, index + 1,
                                       routeInfo->originalRouteInfos))
            return true;
    }
    return false;
}
 ...
 */
void Ipv4NetworkConfigurator::checkOriginalRoutes(const RoutingTableInfo &routingTableInfo,
                                                  const RoutingTableInfo &originalRoutingTableInfo)
{
    ...
    for (auto &originalRouteInfo : originalRoutingTableInfo.routeInfos) {
        Ipv4NetworkConfigurator::RouteInfo *matchingRouteInfo =
            routingTableInfo.findBestMatchingRouteInfo(originalRouteInfo->destination);
        Ipv4NetworkConfigurator::RouteInfo *matchingOriginalRouteInfo =
            originalRoutingTableInfo.findBestMatchingRouteInfo(originalRouteInfo->destination);
 ...
 */
void Ipv4NetworkConfigurator::findLongestCommonDestinationPrefix(
    uint32_t destination1, uint32_t netmask1, uint32_t destination2, uint32_t netmask2,
    uint32_t &destinationOut, uint32_t &netmaskOut)
{
    netmaskOut = 0;
    destinationOut = 0;
    for (int bitIndex = 31; bitIndex >= 0; bitIndex--) {
        uint32_t mask = 1 << bitIndex;
        if ((destination1 & mask) == (destination2 & mask) && (netmask1 & mask) != 0
            && (netmask2 & mask) != 0) {
            netmaskOut |= mask;
 ...
 */
void Ipv4NetworkConfigurator::addOriginalRouteInfos(
    RoutingTableInfo &routingTableInfo, int begin, int end,
    const std::vector<RouteInfo *> &originalRouteInfos)
{
    for (auto &originalRouteInfo : originalRouteInfos) {
        Ipv4NetworkConfigurator::RouteInfo *matchingRouteInfo =
 ...
 */
bool Ipv4NetworkConfigurator::tryToMergeTwoRoutes(RoutingTableInfo &routingTableInfo, int i, int j,
                                                  RouteInfo *routeInfoI, RouteInfo *routeInfoJ)
{
    // determine longest shared address prefix and netmask by iterating through bits from left to right
    uint32_t netmask;
    uint32_t destination;
    findLongestCommonDestinationPrefix(routeInfoI->destination, routeInfoI->netmask,
                                       routeInfoJ->destination, routeInfoJ->netmask, destination,
                                       netmask);
    ...
    if (!interruptsAnyOriginalRoute(routingTableInfo, j + 1, i, routeInfoJ->originalRouteInfos)
        && // check that original routes on J are not interrupted between J and I
        !interruptsAnyOriginalRoute(routingTableInfo, i + 1, m, routeInfoJ->originalRouteInfos)
        && // check that original routes on J are not interrupted between I and M
        !interruptsAnyOriginalRoute(routingTableInfo, i + 1, m, routeInfoI->originalRouteInfos)
        && // check that original routes on I are not interrupted between I and M
        !interruptsSubsequentOriginalRoutes(
            routingTableInfo, m)) // check that the original routes after M are not interrupted by M
    {
        ...
        for (int k = m + 1; k < (int)routingTableInfo.routeInfos.size(); k++) {
            Ipv4NetworkConfigurator::RouteInfo *followingRouteInfo =
                routingTableInfo.routeInfos.at(k);
            for (int l = 0; l < (int)followingRouteInfo->originalRouteInfos.size(); l++) {
                Ipv4NetworkConfigurator::RouteInfo *originalRouteInfo =
                    followingRouteInfo->originalRouteInfos.at(l);
                if (!((originalRouteInfo->destination ^ mergedRouteInfo->destination)
                      & mergedRouteInfo->netmask)) {
                    followingRouteInfo->originalRouteInfos.erase(
                        followingRouteInfo->originalRouteInfos.begin() + l);
...
bool Ipv4NetworkConfigurator::tryToMergeAnyTwoRoutes(RoutingTableInfo &routingTableInfo)
{
    for (int i = 0; i < (int)routingTableInfo.routeInfos.size(); i++) {
        ...
        for (int j = i - 1; j >= 0; j--) {
            ...
            if (routeInfoI->color == routeInfoJ->color
                && routesCanBeNeighbors(routingTableInfo.routeInfos, i, j)) {
...
void Ipv4NetworkConfigurator::optimizeRoutes(std::vector<Ipv4Route *> &originalRoutes)
{
    ...
    std::vector<Ipv4Route *>
        colorToRoute; // a mapping from color to route action (interface, gateway, metric, etc.)
    RoutingTableInfo
        originalRoutingTableInfo; // a copy of the original routes in the optimizer's format
    ...
    for (auto &originalRoute : originalRoutes) {
        ...
        RouteInfo *originalRouteInfo = new RouteInfo(
            color, originalRoute->getDestination().getInt(), originalRoute->getNetmask().getInt());
...
bool Ipv4NetworkConfigurator::getInterfaceIpv4Address(L3Address &ret,
                                                      NetworkInterface *networkInterface,
                                                      bool netmask)
{
    auto it = topology.interfaceInfos.find(networkInterface->getId());
