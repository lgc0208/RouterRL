...
LastEditTime : 2024-09-24 08:34:16
FilePath     : /root/RouterRL/modules/router_rl/multipath_env.py
...
class MultipathEnv(BaseEnv):
    """The interface file between Python and OMNeT++/INET with multipath support."""
    ...
    def init_ned_info(self, ned_path: str) -> Tuple[int, List[List[int]]]:
        ...
        Returns:
            Tuple[int, List[List[int]]]:
                [Number of routers, Topo adjacency matrix]
        """
    ...
    def find_all_shortest_paths(self, src: int, dst: int, max_paths=1000) -> List[List[int]]:
        ...
        Args:
            ...
            max_paths (int, optional): Max number of shortest paths. Defaults to 1000.
    ...
    def get_shortest_paths(self) -> str:
        """Get all shortest paths and converts to routing table.

        Returns:
            str: ECMP routing table in formats of RouterRL.
        """
        init_routing_table = ""
        ...
        for src in range(self.node_num):
            for dst in range(self.node_num):
                if src != dst:
                    ...
                    if all_paths:
                        ratio = 1 / len(all_paths)
                        for path in all_paths:
                            hop_str = ".".join(map(str, path))
                            init_routing_table += f"{src},{dst},{hop_str},{ratio};"
