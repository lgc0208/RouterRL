 ...
 */
class INET_API Ipv4 : public OperationalBase,
                      public NetfilterBase,
...
protected:
    ...
    // ARP related
    PendingPackets
        pendingPackets; // map indexed with IPv4Address for outbound packets waiting for ARP resolution

    // statistics
    int numMulticast = 0;
    int numLocalDeliver = 0;
    int numDropped =
        0; // forwarding off, no outgoing interface, too large but "don't fragment" is set, TTL exceeded, etc
    int numUnroutable = 0;
    int numForwarded = 0;
...
protected:
    ...
    virtual const NetworkInterface *
    getShortestPathInterfaceToSource(const Ptr<const Ipv4Header> &ipv4Header) const;
...
protected:
     ...
     */
    virtual const NetworkInterface *
    determineOutgoingInterfaceForMulticastDatagram(const Ptr<const Ipv4Header> &ipv4Header,
    ...
    virtual MacAddress resolveNextHopMacAddress(cPacket *packet, Ipv4Address nextHopAddr,
...
public:
    ...
    virtual void handleRegisterService(const Protocol &protocol, cGate *gate,
                                       ServicePrimitive servicePrimitive) override;
